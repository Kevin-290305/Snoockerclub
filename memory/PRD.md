...
- 2026-09-25: CORREÇÃO DA VERCEL — a causa do erro do usuário é estrutural: a Vercel publicava só o site estático, sem servidor para conferir a senha. Criadas funções serverless Python em Snoockersalto/api/ (campeonatos GET+seed, inscricoes POST, admin/login, admin/campeonatos GET+POST, admin/campeonatos/[id] PATCH+DELETE, admin/inscricoes GET, admin/inscricoes/[id] DELETE; cada arquivo autossuficiente) + requirements.txt e vercel.json na raiz do pacote + passo a passo Vercel/Atlas no LEIA-ME. FALHA GRAVE EVITADA: login.py inicial retornava 200 com ok:false para senha errada (qualquer senha abriria o painel) — corrigido para 401. Testado localmente no pod com Mongo real: criar/listar/inscrever/encerrar/excluir tudo 200/401 conforme esperado. Frontend não precisou mudar (mesmas rotas /api).
- 2026-09-25: Sondagem ao deploy real do usuário (snookerclub-976fea3x-ke29032005-3998s-projects.vercel.app): /api/admin/login e /api/campeonatos = 404 (api/ não foi publicado) e admin.html sem as strings da versão nova (deploy antigo, só estático). Corrigido também vazamento de UI: .admin-panel com display:grid ignorava o atributo hidden e o painel aparecia abaixo do portão de senha sem login — adicionado [hidden]{display:none!important} (verificado: painel none antes do login, grid depois). Pacote Snoockersalto + zip ressincronizados.
- 2026-09-25: PEDIDO DO USUÁRIO — REMOVIDO O BANCO DE DADOS POR COMPLETO. Nova arquitetura 100% estática: campeonatos vivem no arquivo js/campeonatos.js (window.CAMPEONATOS, editável à mão); admin.html virou gerador — portão de senha conferido no navegador (Base64 na linha SENHA_CODIFICADA = FlavioSnoocker2026), formulário gera o conteúdo completo do arquivo para copiar e colar; inscrições vão SOMENTE pelo WhatsApp (POST ao backend removido); removidos api/, requirements.txt, vercel.json e backend/ do pacote Snoockersalto (ficou 100% estático: 11 arquivos, funciona até abrindo index.html com duplo clique); backend do pod simplificado (só health + site estático na porta 8001) e ADMIN_PASSWORD removida do .env; LEIA-ME reescrito. Corrigido durante o build: template em cache do dev server (restart do frontend necessário). Verificado: home renderiza o card do campeonatos.js com ZERO chamadas /api; seletor da inscrição populado pelo arquivo; admin gera código com 2 campeonatos; mobile 390 sem overflow; senha FlavioSnoocker2026 abre o painel. Publicar campeonato agora = gerar código no painel → colar em js/campeonatos.js → salvar → deploy.
