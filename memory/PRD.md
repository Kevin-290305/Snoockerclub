...
- 2026-09-25: Segunda sondagem: o domínio antigo do usuário agora dá DEPLOYMENT_NOT_FOUND em tudo (deploy removido/projeto recriado). O print do usuário mostrou a versão nova do admin.html respondendo 401 com a senha certa = ADMIN_PASSWORD ausente/não aplicada na Vercel (variáveis só valem em deploy novo). Mensagens do portão encurtadas a pedido do usuário ("Senha incorreta. Tente novamente.") e parágrafo de dica sobre backend/.env removido do portão; verificado no preview. Pacote + zip sincronizados.
